...
vp.validateEmail = function(type, val) {
	...
	var regex = new RegExp(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,63}$/i);
	return regex.test(val);
};
...
{
	if (jQuery.fn.tipsy) {
		var liveContainerSelector = '.vp-metabox, .vp-wrap';
		jQuery('.vp-js-tipsy.description').tipsy({ live: liveContainerSelector, gravity : 'e' });
		jQuery('.vp-js-tipsy.slideinput').tipsy({ live: liveContainerSelector, trigger : 'focus' });
		jQuery('.vp-js-tipsy.image-item').tipsy({ live: liveContainerSelector });
	}
};
vp.init_tipsy();
